// components/mapping/GhlConnectionEditor.tsx — the GHL↔GHL (contact⇄company) connection editor.
// Same load/suggest/save behavior as the old mappings page, now rendered with the shared
// ToolObjectBand + ConnectionTable (two-way). Preserves mirrorDown/holdValues on save.

import { useCallback, useEffect, useMemo, useState } from 'react';
import ToolObjectBand from './ToolObjectBand';
import ConnectionTable, { type ConnRow, type ConnStatusFilter } from './ConnectionTable';
import type { FieldOptions } from './ConnectionTable';
...
function toRow(m: ResolvedFieldMapping): ConnRow {
...
export default function GhlConnectionEditor() {
  ...
  const [status, setStatus] = useState<ConnStatusFilter>('all');
  ...
  const loadSync = useCallback(async () => {
    ...
    setIssues(data.issues ?? []); setVersion(data.version ?? 0); setUpdatedAt(data.updatedAt ?? ''); setDirty(false);
  }, []);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setSource({ scalars: cat.contact.scalars, fields: cat.contact.fields });
        setTarget({ scalars: cat.business.scalars, fields: cat.business.fields });
        await loadSync();
      } catch (e: any) { setLoadError(e?.message ?? 'Failed to load'); } finally { setLoading(false); }
    })();
  }, [loadSync]);
  ...
  async function autoSuggest() {
    ...
    try {
      ...
      const additions = suggestions.filter((s) => !have.has(`${s.contactKey}→${s.businessKey}`))
        .map((s) => ({ sourceKey: s.contactKey, targetKey: s.businessKey, direction: s.direction, mirrorDown: s.mirrorDown } as ConnRow));
  ...
  async function save() {
    ...
    try {
      ...
      await loadSync();
      setSaveMsg(`Saved v${data.version} · ${data.count} mappings · live now.`);
    ...
    <>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20, marginBottom: 18 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 26, letterSpacing: '-.02em', margin: 0, color: 'var(--text)' }}>Contact ⇄ Company</h1>
          <p style={{ margin: 0, fontSize: 14, color: 'var(--gray-500)' }}>
            {loading ? 'Loading mappings…' : <><b style={{ color: 'var(--text)', fontWeight: 700 }}>{activeCount}</b> of {rows.length} fields syncing — set direction and transforms per field.{version ? <span style={{ color: 'var(--gray-400)' }}> · v{version}{updatedAt ? ` · updated ${new Date(updatedAt).toLocaleDateString()}` : ''}</span> : null}</>}
          </p>
        </div>
      ...
      <ToolObjectBand source={{ tool: 'ghl', object: 'contact' }} target={{ tool: 'ghl', object: 'business' }} />
      ...
      {!loading && !loadError && (
        <>
          {(errorCount > 0 || warnCount > 0) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', borderRadius: 10, marginBottom: 14, fontSize: 13.5, fontWeight: 600, background: errorCount ? '#fde8e8' : 'var(--brand-tint)', color: errorCount ? '#b42318' : 'var(--yellow-700)', border: `1px solid ${errorCount ? '#f5c2c0' : 'var(--brand)'}` }}>
              <i className={`fa-solid ${errorCount ? 'fa-circle-exclamation' : 'fa-triangle-exclamation'}`} />
              {errorCount > 0 && <span>{errorCount} error{errorCount > 1 ? 's' : ''}</span>}{errorCount > 0 && warnCount > 0 && <span>·</span>}{warnCount > 0 && <span>{warnCount} warning{warnCount > 1 ? 's' : ''}</span>}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 2, background: 'var(--gray-100)', padding: 3, borderRadius: 999 }}>
              {FILTERS.map((f) => { const on = status === f.id; return (
                <button key={f.id} type="button" onClick={() => setStatus(f.id)} style={{ padding: '6px 13px', borderRadius: 999, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 12.5, background: on ? 'var(--surface)' : 'transparent', color: on ? 'var(--text)' : 'var(--gray-500)', boxShadow: on ? 'var(--shadow-xs)' : 'none' }}>{f.label}</button>
              ); })}
            </div>
          ...
          <ConnectionTable rows={rows} source={source} target={target} oneWay={false} destTone="teal" sourceLabel="Source field · Contact" targetLabel="GHL destination · Company" disabled={saving} filter={{ query, status }} onChange={(r) => { setRows(r); setDirty(true); setSaveMsg(''); setSaveErr(''); }} />
        </>
      )}
    </>
  );
